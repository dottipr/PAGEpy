...
def evaluate_model(input_model, input_data):
    ...
    x_values = np.arange(
        1, len(input_model.train_accuracy_list) + 1) * report_frequency

    # Plot train accuracy
    axs[0].plot(x_values, input_model.train_accuracy_list,
    ...
    # Plot train auc
    axs[1].plot(x_values, input_model.train_auc_list,
    ...
    # Plot test accuracay
    axs[2].plot(x_values, input_model.test_accuracy_list,
    ...
    # Plot test auc
    axs[3].plot(x_values, input_model.test_auc_list,
...
def plot_pso_row_averages(df):
    ...
    # Plot row averages
    plt.plot(row_averages, marker='o', linestyle='-',
             color='b', label='Average AUC')

    # Plot the shaded region representing the standard deviation
    plt.fill_between(range(len(df)), row_averages - row_std, row_averages +
                     row_std, color='b', alpha=0.2, label='Standard Deviation')
        ...
        for p in range(len(current_list) - 1):
            ...
            for pop_member in relevant_comparisons:
                distance = hamming_distance(
                    input_dict[i][p], input_dict[i][pop_member])
                hamming_generation_values.append(distance)
        current_hamming_average = sum(
            hamming_generation_values) / len(hamming_generation_values)
        hamming_averages.append(current_hamming_average)
        gen.append(i)

    # Plot gen averages for hamming distance
    plt.plot(hamming_averages, marker='o', linestyle='-',
             color='b', label='Average AUC')
...
def plot_sorted_frequencies(loaded_dict, loaded_df):
    ...
    plt.plot(sorted_frequencies_1, marker='o',
             linestyle='-', color='blue', label="First Gen")
    plt.plot(sorted_frequencies_2, marker='s',
             linestyle='--', color='red', label="Latest Gen")
